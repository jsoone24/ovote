...
function instantiate(version) {
    BlockchainService.instantiateChaincode(
        "org1",
        CONFIG.chaincode.id,
        ...
        .then(
            ...
            (err) => {
                logger.error(
                    "Failed to instantiate chaincode on the channel. " + err.stack ? err.stack : err
                );
            }
        )
        .then(() => {
            logger.info(
                "Successfully slept 5s to wait for chaincode instantiate to be completed and committed in all peers"
            );
        });
}
