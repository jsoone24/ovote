    ...
    async.each(
        ...
        (org, next) => {
            console.log(
                "service.install org:" +
                    ...
                    " chaincode.id:" +
                    CONFIG.chaincode.id +
                    " CONFIG.chaincode.path:" +
                    CONFIG.chaincode.path +
                    " ver:" +
                    ver
            );
            let installprom = service
                .install(org, CONFIG.chaincode.id, CONFIG.chaincode.path, ver, "golang", true)
                .then(
                    (res) => {
        ...
        (err) => {
            if (err) {
                ...
                let instantprom = service
                    .instantiate("org1", update, CONFIG.chaincode.id, ver, initFnc, initArgs, true)
                    .then(
                        (res) => {
                            cnt++;
                        ...
                        (err) => {
                            cnt++;
                            console.log(
                                "Failed to instantiate: " + JSON.stringify(err) + " ver:" + ver
                            );
                        }
                    );
